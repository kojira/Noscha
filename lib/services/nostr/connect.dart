// from https://github.com/0xchat-app/0xchat-core/blob/main/lib/src/common/network/connect.dart
...
/// send event callback
typedef OKCallBack = void Function(Ok ok, String relay);

/// request callback
typedef EventCallBack = void Function(Event event, String relay);
typedef EOSECallBack = void Function(
    String requestId, Ok ok, String relay, List<String> unCompletedRelays);
  ...
  void _checkTimeout() {
    ...
    for (var eventId in okMapKeys) {
      ...
      if (now - start > timeout * 1000) {
        // timeout
        Ok ok = Ok(eventId, false, 'Time Out');
    ...
    for (var subscriptionId in requestMapKeys) {
      ...
      if (requestsMap[subscriptionId] != null && callBack != null) {
        ...
        if (start > 0 && now - start > timeout * 1000 * 2) {
          // timeout
          Ok ok = Ok(subscriptionId, false, 'Time Out');
